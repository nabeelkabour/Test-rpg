...
{
	_page_state->update(this);
	return true;
}

int round(Lifeform& char1, int _char1_dmg, Lifeform& char2, int _char2_dmg)
{
	//get player input
	//player attack
...
int player_fight(Player& player, Lifeform& char2)
{
	while (true)
	{
		...
		case '4':
			...
			player.select_item(slot_val);
			//display_inventory(player);
			break;
		}

		int fight_state = round(player, player_damage, char2, char2.get_damage());
